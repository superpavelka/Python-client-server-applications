import sys
import logging
import logs.server_config
import logs.client_config
import traceback
import inspect

# метод определения модуля, источника запуска.
# Метод find () возвращает индекс первого вхождения искомой подстроки,
# если он найден в данной строке.
# Если его не найдено, - возвращает -1.
if sys.argv[0].find('client.py') == -1:
    # если не клиент то сервер!
    LOGGER = logging.getLogger('server')
else:
    # если не сервер, то клиент
    LOGGER = logging.getLogger('client')

def log_extra_info(f_to_log):
    """Функция-декоратор"""
    def log_save(*args, **kwargs):
        ret = f_to_log(*args, **kwargs)
        LOGGER.debug(f'Была вызвана функция {f_to_log.__name__} c параметрами {args}, {kwargs}. '
                     f'Вызов из модуля {f_to_log.__module__}. Вызов из'
                     f' функции {traceback.format_stack()[0].strip().split()[-1]}.'
                     f'Вызов из функции {inspect.stack()[1][3]}')
        return ret
    return log_save